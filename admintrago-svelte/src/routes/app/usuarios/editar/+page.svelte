<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      ciudad = usuario.USU_CIUDAD || '';
      sector = usuario.USU_SECTOR_ENTREGA || '';
      genero = usuario.USU_GENERO || '';
      fechaRegistro = usuario.USU_FECHA_REGISTRO
  ...
  async function guardarCambios(e) {
    ...
    try {
      ...
      const resLogin = await fetch('https://eltragolocorest.runasp.net/api/Login/Actualizar', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
